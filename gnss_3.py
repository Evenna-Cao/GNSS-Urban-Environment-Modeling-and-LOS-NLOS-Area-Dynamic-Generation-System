...
def load_ephemeris_with_library(file_path):
    ...
    """
    try:
        # load() 会自动识别 RINEX 版本并返回 Dataset
        nav = gr.load(file_path)
        print(f"成功加载星历文件: {file_path}")
        return nav
...
def get_eph_dict_from_dataset(nav_ds, prn):
    """
    从 georinex 的 Dataset 中提取特定卫星的参数
    """
    # 修正点：直接使用 prn 变量，不需要再加 .values
    sat_data = nav_ds.sel(sv=prn).dropna(dim='time', how='all').isel(time=-1)

    eph = {
        'PRN': str(prn),  # 这里 prn 已经是字符串了
        'toe': float(sat_data['Toe']),
        'sqrt_a': float(sat_data['sqrtA']),
        'e': float(sat_data['Eccentricity']),
        'm0': float(sat_data['M0']),
        'delta_n': float(sat_data['DeltaN']),
        'omega': float(sat_data['omega']),
        'omega0': float(sat_data['Omega0']),
        'omega_dot': float(sat_data['OmegaDot']),
        'i0': float(sat_data['Io']),
        'idot': float(sat_data['IDOT']),
        'cus': float(sat_data['Cus']),
        'cuc': float(sat_data['Cuc']),
        'crs': float(sat_data['Crs']),
        'crc': float(sat_data['Crc']),
        'cis': float(sat_data['Cis']),
...
if __name__ == "__main__":
    ...
    if nav_ds is not None:
        # 获取文件中所有的卫星编号 (比如 G01, G02...)
        all_svs = nav_ds.sv.values

        for sv in all_svs:
            # 过滤非 GPS 卫星 (georinex 会把所有星座都读出来，如果你只算 GPS)
            if not str(sv).startswith('G'): continue
            ...
            try:
                ...
                # 轨道计算与坐标转换
                sat_ecef = gps_orbit_calculation(eph, eph['toe'])
                sat_enu = ecef_to_enu(sat_ecef[0], sat_ecef[1], sat_ecef[2], 22.3, 114.2, 50.0)

                # 仅处理地平线以上的卫星
                if sat_enu[2] > 0:
                    sat_obj = Satellite(str(sv), sat_enu)
                    vis_matrix = sim.check_visibility(sat_obj.position)
                    plot_and_save_results(output_folder, sim, sat_obj, vis_matrix)
                else:
                    print(f"  - {sv} 在地平线下，跳过。")
            except Exception as e:
                print(f"处理卫星 {sv} 时出错: {e}")
